...
PreparedStatement pt = con.prepareStatement("select * from booked where username=?");
...
while(ras.next())
{
	String c=ras.getString("carname");
	 String status=ras.getString("status");
	
	session.setAttribute("c",c);
	if(status.equals("Pending")){
	%> <p style="color:gold; font-weight:bold">Status: <%out.println(status); %></p><%
	}
	else if(status.equals("Approved")){
		%> <p style="color:green; font-weight:bold">Status: <%out.println(status); %></p><%
		}
	else if(status.equals("Declined")){
		%> <p style="color:red; font-weight:bold">Status: <%out.println(status); %></p><%
		}
...
ps.setString(1,carname);
	...
	width: 50%;
	font-size: 18px;
	border-radius: 50px;"  formaction="admincancel.jsp">cancel</button></p>
